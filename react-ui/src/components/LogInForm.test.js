...
describe('SignupForm component', () => {
  ...
  test('loading of dashboard page from log in', async () => { // doesn't cover statement
    //Arrange
    const { rerender, getByPlaceholderText, getByDisplayValue, queryByPlaceholderText } = render(
      <BrowserRouter>
        <LogInForm />
      </BrowserRouter>);
    ...
    const submit = getByDisplayValue("Log In!");
    ...
    rerender(<BrowserRouter>
      <LogInForm loginParams={testWorker} />
    </BrowserRouter>);
    ...
    //Assert
    rerender(<BrowserRouter>
      <Route exact path="/login" render={props => (<Redirect to={`/dashboard/${'joshuabrooks@gmail.com'}/${'worker'}/${'Joshua'}/${8000000}/${'eeJAQr3wEC6CJZROFJTY'}`} Component={Dashboard} />)}></Route>
    </BrowserRouter>)
